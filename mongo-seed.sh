...
mongo mongodb/library --eval 'db.releases.aggregate([{$addFields: {label: {$arrayElemAt: ["$label-info", 0]}}},{$group: {_id: "$label.label.id",labelName: {$first: "$label.label.name"},disambiguation: {$first: "$label.label.disambiguation"},releases: {$push: {releaseID: "$_id",releaseGroupID: "$KEXPReleaseGroupMBID",title: "$title",catalogNumber: "$label.catalog-number",}}},},{$out: "labels"}])'
